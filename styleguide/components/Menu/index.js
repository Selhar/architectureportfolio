      ...
      items.map(item => (
        <li key={"li" + item}>
          <a href={item[1]} key={item} 
            className={"mr-4 font-bold cursor-pointer text-white hover:text-yellow"}
...
export default ({items, open, setOpen, classes}) => (
  <div className={classes}>
    <Menu items={items} classes={"sm:flex hidden text-white flex-row flex-wrap uppercase justify-center tracking-wider p-4"} />
    <HamburguerIcon classes={"sm:hidden"} open={open} setOpen={setOpen} />
    <div className={"w-screen h-screen left-outofbounds flex flex-row justify-between top-0 absolute h-16 bg-orange w-full p-4 transition-left duration-500 ease-out" + (open ? " left-0" : "")} >  
      ...
      <span className="font-bold cursor-pointer text-white pd-2 pl-2" onClick={() => setOpen(!open)}>X</span>
    </div>
  </div>
)